...
import { Search, Filter, Download, MoreHorizontal, UserCheck, AlertTriangle, Building2, MapPin, Eye } from 'lucide-react';
import MerchantDetail from '../merchant/MerchantDetail';

const MerchantListPage = ({ merchants }) => {
  ...
  return (
    <div className="space-y-6 animation-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Data Merchant</h1>
          <p className="text-slate-500 text-sm">Database seluruh mitra merchant terdaftar.</p>
        </div>
                ...
                filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${item.status === 'Active' ? 'bg-green-100 text-green-700 border-green-200' :
                          item.status === 'Suspend' ? 'bg-red-100 text-red-700 border-red-200' :
                            'bg-blue-100 text-blue-700 border-blue-200'
                        }`}>
                        {item.status === 'Active' && <UserCheck size={12} />}
                        {item.status === 'Suspend' && <AlertTriangle size={12} />}
                        {item.status}
                      </span>
                    </td>
